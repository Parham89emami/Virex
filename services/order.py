...
async def create_order_for_user(session: AsyncSession, user: User, plan: VPNPlan) -> Order:
    ...
    if existing is not None:
        return existing
    order = Order(
        order_uid=f"VIR-{__import__('uuid').uuid4().hex[:8].upper()}",
        user_id=user.id,
        plan_name=plan.name,
...
async def cancel_order_for_user(session: AsyncSession, telegram_id: int, order_id: int) -> Order | None:
    result = await session.execute(
        select(Order).join(Order.user).options(selectinload(Order.payment)).where(
            Order.id == order_id,
...
async def cancel_all_orders_for_user(session: AsyncSession, telegram_id: int) -> int:
    """Cancel all cancellable orders owned by the user; never delete history."""
    result = await session.execute(
        select(Order).join(Order.user).options(selectinload(Order.payment)).where(
            User.telegram_id == telegram_id,
            Order.status.in_(["pending", "pending_review"]),
