...
async def coupon_price(s,code,user_id,price):
    ...
    if c.expires_at and c.expires_at.replace(tzinfo=timezone.utc)<datetime.now(timezone.utc):return price,None,"کد تخفیف منقضی شده است."
...
async def create_order(s,user,product,price=None,coupon=None):
    ...
    try: await s.commit()
    except IntegrityError: await s.rollback(); raise
    await s.refresh(o); return o
async def pending_order(s,telegram_id): return (await s.execute(select(Order).join(Order.user).where(User.telegram_id==telegram_id,Order.payment_status=="pending").order_by(Order.created_at.desc()).limit(1))).scalar_one_or_none()
async def user_orders(s,telegram_id): return list((await s.execute(select(Order).join(Order.user).where(User.telegram_id==telegram_id).order_by(Order.created_at.desc()))).scalars().all())
...
async def debit_wallet(s,user,amount,description):
    if amount<0 or user.wallet_balance<amount:return False
    user.wallet_balance-=amount;s.add(WalletTransaction(user_id=user.id,amount=-amount,transaction_type="debit",description=description));return True
async def approve_and_deliver(s,order_id):
    ...
    result=await s.execute(update(VPNConfig).where(VPNConfig.id==c.id,VPNConfig.status=="available").values(status="sold",sold_to_user_id=o.user_id,order_id=o.id,sold_at=datetime.now(timezone.utc)))
    if result.rowcount!=1:await s.rollback();return o,None
    o.config_id=o.status="completed";o.payment_status="approved";o.completed_at=datetime.now(timezone.utc);await s.commit();await s.refresh(o);return o,c
