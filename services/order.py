...
async def create_order_for_user(session: AsyncSession, user: User, plan: VPNPlan) -> Order:
    order = Order(order_uid=f"VIR-{uuid.uuid4().hex[:8].upper()}", user_id=user.id, plan_name=plan.name, traffic_gb=plan.traffic_gb, duration_days=plan.duration_days, price_toman=plan.price_toman)
    session.add(order)
    await session.commit()
...
async def get_latest_pending_order_for_user(session: AsyncSession, telegram_id: int) -> Order | None:
    result = await session.execute(select(Order).join(Order.user).where(User.telegram_id == telegram_id, Order.payment_status == "pending").order_by(Order.created_at.desc()).limit(1))
...
async def get_pending_orders(session: AsyncSession) -> list[Order]:
    result = await session.execute(select(Order).options(selectinload(Order.user)).where(Order.payment_status == "pending_review").order_by(Order.created_at.asc()))
    return list(result.scalars().all())
