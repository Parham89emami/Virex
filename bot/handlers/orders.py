...
async def show_plans(update,context):
 ...
 await update.message.reply_text('📦 محصول موردنظر را انتخاب کنید:',reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(f'{p.volume_gb} گیگ | {p.duration_days} روز | {p.price:,} تومان',callback_data=f'product:{p.id}')] for p in ps]) if ps else get_main_menu_keyboard())
async def select_plan(update,context):
 q=update.callback_query
 if not q or not update.effective_user:return
 await q.answer(); pid=int((q.data or '').split(':',1)[1])
...
async def handle_coupon_input(update,context):
 ...
 async with async_session_factory() as s:
  ...
  amount,coupon,error=(p.price,None,None) if code.lower() in {'ندارم','no','none'} else await coupon_price(s,code,u.id,p.price)
...
async def payment_choice(update,context):
 ...
 async with async_session_factory() as s:
  ...
  if method=='wallet':
   ...
   o=await create_order(s,u,p,amount,coupon);o.status=o.payment_status='pending_review';await s.commit();await q.edit_message_text('✅ پرداخت کیف پول ثبت شد و پس از بررسی ادمین تحویل می‌شود.')
